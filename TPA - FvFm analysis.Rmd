...
```{r}
FvFm.df <- read_xlsx("./GCB dataset.xlsx", sheet = 4)

#Relevel nutrient treatments
FvFm.df$Growth_temperature <- ordered(FvFm.df$Growth_temperature, levels = c('21', '23', '25', '27', '29', '31', '33'))
FvFm.df$Nutrient_treatment <- ordered(FvFm.df$Nutrient_treatment, levels = c('IMK', 'NO3_depleted'))
...
Delta_FvFm.plot <- FvFm.df4 %>%
  ...
  stat_pvalue_manual(tukey_hsd.Delta_FvFm, label = "p.adj.signif", y.position = c(-0.01, 0.02, -0.01, 0, 0.01), tip.length = 0.01) +
  scale_x_discrete('Experimental evolution treatment', expand =c(0,0)) +
  scale_y_continuous(expression(bold(Δ~F[v]/F[m])), expand =c(0,0), limits = c(-0.15, 0.05)) +
  scale_colour_manual(name = 'Experimental evolution treatment', values=c("#8B0000","#E4B031", "#26727B", "#4770B3", "#58595b")) +
  scale_fill_manual(name = 'Experimental evolution treatment', values=c("#8B0000","#E4B031", "#26727B", "#4770B3", "#58595b")) +
  theme_bw() +
...
FvFm_DHW.plots <- cowplot::plot_grid(DHW_FvFm.plot,
                                     DHW_Delta_FvFm.plot,
                                     labels = c("A", "B"),
                                     ncol = 2,
                                     align = "v",
...
ggsave(plot = FvFm_DHW.plots, "TPA - FvFM by DHW plots.pdf", 
       path = "./",
       width = 10,
       height = 5,
       units = 'in',
