...
PR.df <- read_xlsx("./Physiological datasets - EE & TPA.xlsx", sheet = 7) %>%
  mutate(phase = Phase) %>%
...
```{r}
#Respiration plots
Rd.df_import <- read_xlsx("./Physiological datasets - EE & TPA.xlsx", sheet = 8) %>%
  filter(Lineage != 'Blank') %>%
...
Pnet.df_import <- read_xlsx("./Physiological datasets - EE & TPA.xlsx", sheet = 8) %>%
  filter(Lineage != 'Blank') %>%
