...
GC.plot <- TPA.df %>%
  ...
  geom_vline(data=filter(TPA.df, Growth_cycle == 2 & Growth_temperature == 21), aes(xintercept = 18), colour="black", size = 0.5, linetype = 'solid') +
  geom_vline(data=filter(TPA.df, Growth_cycle == 2 & Growth_temperature == 23), aes(xintercept = 14), colour="black", size = 0.5, linetype = 'solid') +
  geom_vline(data=filter(TPA.df, Growth_cycle == 2 & Growth_temperature == 25), aes(xintercept = 10), colour="black", size = 0.5, linetype = 'solid') +
...
for(i in 1:nrow(TPC.thomas2017)){
  ...
  temp_fit <- nlsLM(mumax ~ thomas_2017(temp = Growth_temperature, a, b, c, d, e),
               ...
               lower = get_lower_lims(temp_data$Growth_temperature, temp_data$mumax, model_name = 'thomas_2017'),
               upper = get_upper_lims(temp_data$Growth_temperature, temp_data$mumax, model_name = 'thomas_2017'))
  boot <- Boot(temp_fit, f = function(x){unlist(calc_params(x))}, labels = names(calc_params(temp_fit)), R = 20, method = 'residual') %>%
  confint(., method = 'bca') %>%
  as.data.frame() %>%
...
TPC.EE_param.plot <- TPC.EE_param %>%
  ...
  scale_colour_manual(labels = treatment.labs, values=c("#58595b","#4770B3", "#26727B", "#E4B031", "#8B0000")) +
  scale_fill_manual(labels = treatment.labs, values=c("#58595b","#4770B3", "#26727B", "#E4B031", "#8B0000")) +
  facet_wrap(.~param, ncol = 2, scales = 'free', labeller = param_labels) +
  #coord_flip() +
...
```{r}
TPC.EE.plots <- ggarrange(TPC.EE_CI.plot,
...
ggsave(plot = TPC.CI.plot, "TPA - TPC with CI.pdf", 
       path = "./", 
       width=12.5,
       height=10,
       units='in',
...
ggsave(plot = TPC.EE.plots, "TPA - TPC EE with CI.pdf", 
       path = "./", 
       width=12.5,
       height=10,
       units='in',
...
ggsave(plot = TPC.param_CI.plot, "TPA - TPC param with CI.pdf", 
       path = "./", 
       width=12.5,
       height=10,
       units='in',
