...
#Re-level nutrient treatments
TPA.df$Growth_temperature <- ordered(TPA.df$Growth_temperature, levels = c('21', '23', '25', '27', '29', '31'))
TPA.df$Nutrient_treatment <- ordered(TPA.df$Nutrient_treatment, levels = c('IMK', 'NO3_depleted'))

#Prepare labels for plot facets
nutrient.labs <- c("Nitrate replete", "Nitrate depleted")
names(nutrient.labs) <- c('IMK', "NO3_depleted")
...
#Growth cycle 1
GC1.df <- TPA.df %>%
  filter(Nutrient_treatment == 'IMK' & Growth_cycle == 1) %>%
  filter(CellD > 0) %>%
  mutate(log10CellD = log10(CellD))

#Growth cycle 2
GC2.df <- TPA.df %>%
  filter(Nutrient_treatment == 'IMK' & Growth_cycle == 2) %>%
  filter(Growth_temperature != 21 ) %>% #Remove t27 and t30 timepoints for TP3

GC2_21C.df <- TPA.df %>%
  filter(Nutrient_treatment == 'IMK' & Growth_cycle == 2 & Growth_temperature == 21) %>%
...
GrowthR.nls_best_model <- GrowthR.nls_df %>%
  ...
  ungroup() %>%
  count(model_name) #Buchanan_no_lag overwhelmingly provides fit
...
GrowthR.newdata <- tibble(Growth_temperature = seq(min(TPC.df$Growth_temperature), max(TPC.df$Growth_temperature), length.out = 42))
...
TPC.plot <- GrowthR.preds_mult %>%
  ...
  scale_y_continuous(expression(bold(Growth~rate~(d^-1))), limits = c(NA,NA)) +
  scale_colour_manual(values=c("#8B0000","#E4B031", "#26727B", "#4770B3", "#58595b")) +
  scale_fill_manual(values=c("#8B0000","#E4B031", "#26727B", "#4770B3", "#58595b")) +
  theme_bw() +
...
#Thomas2017 and kamykowski models appear to provide best fits - proceed with thomas2017 as kamykowski provided strange fits for some TP4 lineages

#Code for model averaging, originally explored but not kept
#Calculate average of models
#GrowthR.ic_ave <- GrowthR.ic %>%
  #filter(model_name %in% c('thomas2017', 'joehnk')) %>%
  #filter(AICc - min(AICc) <= 2) %>% #Filter out model averages which are less than Δ2AICc of the best model
...
#GrowthR.params <- GrowthR.stack %>%
  #mutate(., params = map(fit, calc_params)) %>%
  #dplyr::select(-fit) %>%
  #unnest(params)

#Get averaged parameters based on previously calculated model weights
#GrowthR.params_ave <- left_join(GrowthR.params,  dplyr::select(GrowthR.ic_ave, Lineage, model_name, weight)) %>%
  #dplyr::select(!c(eh, q10, skewness)) %>%
  #na.omit() %>%
  #group_by(Lineage) %>%
...
TPC.treat_plot <- GrowthR.preds_mult %>%
  filter(model_name == 'thomas2017') %>%
  group_by(EE_treatment, Growth_temperature) %>%
...
TPC.lin_plot <- GrowthR.preds_mult %>%
  filter(model_name == 'thomas2017') %>%
  group_by(EE_treatment, Growth_temperature) %>%
  ggplot(aes()) +
  geom_point(aes(Growth_temperature, mumax), TPC.df, size = 1.5, pch = 20, shape = 0) +
...
Extract parameters of interest from model average
```{r}
#Estimate parameters from models
GrowthR.params <- GrowthR.stack %>%
  filter(model_name == 'thomas2017') %>%
  mutate(., params = map(fit, calc_params)) %>%
  dplyr::select(-c(model_name, fit)) %>%
  unnest(params)
```
...
aov.ctmin <- aov(ctmin ~ EE_treatment, data = GrowthR.params[GrowthR.params$Lineage != 'TP1_6', ])
...
boxplot(ctmin ~ EE_treatment, xlab='EE_treatment', ylab='CTmin', data = GrowthR.params[GrowthR.params$Lineage != 'TP1_6', ])
...
tukey_hsd.ctmin <- aov(ctmin ~ EE_treatment, data = GrowthR.params[GrowthR.params$Lineage != 'TP1_6', ]) %>%
  tukey_hsd() %>%
  filter(p.adj < 0.05)
...
Topt.plot <- GrowthR.params %>%
  ...
  stat_pvalue_manual(tukey_hsd.topt, label = "p.adj.signif", y.position = c(28.55, 28.75, 28.95), tip.length = 0.01) +
  scale_y_continuous(expression(bold(T[opt]~(ºC))), expand =c(0,0), limits = c(25, 29.1)) +
...
rmax.plot <- GrowthR.params %>%
  ...
  stat_pvalue_manual(tukey_hsd.rmax, label = "p.adj.signif", y.position = c(0.149, 0.151, 0.141, 0.143, 0.145, 0.147),
                     tip.length = 0.01) +
  scale_y_continuous(expression(bold(r[max]~(day^-1))), expand =c(0,0), limits = c(0.1, 0.153)) +
  scale_x_discrete('Experimental evolution treatment') +
  scale_colour_manual(values=c("#8B0000","#E4B031", "#26727B", "#4770B3", "#58595b")) +
  scale_fill_manual(values=c("#8B0000","#E4B031", "#26727B", "#4770B3", "#58595b")) +
  theme_bw() +
...
CTmax.plot <- GrowthR.params %>%
  ...
  stat_pvalue_manual(tukey_hsd.ctmax, label = "p.adj.signif", y.position = c(31.1), tip.length = 0.01) +
...
CTmin.plot <- GrowthR.params %>%
  ...
  stat_pvalue_manual(tukey_hsd.ctmin, label = "p.adj.signif", y.position = c(21.2, 22.7, 23, 23.3, 22.4, 22.2, 21.2), tip.length = 0.01) +
  scale_y_continuous(expression(bold(CT[min]~(ºC))), expand =c(0,0), limits = c(14, 24)) +
  scale_x_discrete('Experimental evolution treatment') +
  scale_colour_manual(values=c("#8B0000","#E4B031", "#26727B", "#4770B3", "#58595b")) +
  scale_fill_manual(values=c("#8B0000","#E4B031", "#26727B", "#4770B3", "#58595b")) +
  theme_bw() +
...
Tbr.plot <- GrowthR.params %>%
  ...
  stat_pvalue_manual(tukey_hsd.tbr, label = "p.adj.signif", y.position = c(6.8, 7, 7.2, 7.4, 5.7, 5.9), tip.length = 0.01) +
  scale_y_continuous(expression(bold(T[br]~(ºC))), expand =c(0,0), limits = c(3.5, 7.7)) +
  scale_x_discrete('Experimental evolution treatment') +
  scale_colour_manual(values=c("#8B0000","#E4B031", "#26727B", "#4770B3", "#58595b")) +
  scale_fill_manual(values=c("#8B0000","#E4B031", "#26727B", "#4770B3", "#58595b")) +
  theme_bw() +
...
TSM.plot <- GrowthR.params %>%
  ggplot() +
  stat_pvalue_manual(tukey_hsd.tsm, label = "p.adj.signif", y.position = c(5.6, 6, 5.4, 5.2, 5.8), tip.length = 0.01) +
  geom_boxplot(aes(EE_treatment, y = thermal_safety_margin, fill = EE_treatment)) +
  scale_y_continuous(expression(bold(TSM~(ºC))), expand =c(0,0), limits = c(1.5, 6.2)) +
  scale_x_discrete('Experimental evolution treatment') +
  scale_colour_manual(values=c("#8B0000","#E4B031", "#26727B", "#4770B3", "#58595b")) +
  scale_fill_manual(values=c("#8B0000","#E4B031", "#26727B", "#4770B3", "#58595b")) +
  theme_bw() +
...
PCA.matrix <- GrowthR.params %>%
  dplyr::select(-c(EE_treatment, e,eh, q10,  thermal_tolerance, skewness)) %>%
  filter(Lineage != 'TP1_6') %>%
  rename("Topt" = "topt", "CTmin" = "ctmin", "CTmax" = "ctmax", "TSM" = "thermal_safety_margin", "Tbr" = "breadth") %>%
  column_to_rownames(var = "Lineage")

PCA.groups <- GrowthR.params %>% filter(Lineage != 'TP1_6') %>% pull(EE_treatment)
...
PCA.tpc_biplot <- fviz_pca_biplot(PCA.tpc, repel = TRUE,
  ...
  labs(title = "", x = "PC1: 40%", y = "PC2: 34.6%") +
  theme_bw() +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        title = element_blank(),
...
```{r}
TPC_params.plots <- ggarrange(Topt.plot + rremove("xlab"),
                             CTmax.plot + rremove("xlab"),
                             Tbr.plot + rremove("xlab"),
                             TSM.plot + rremove("xlab"),
                             CTmin.plot + rremove("xlab"),
                             rmax.plot + rremove("xlab"),
                             legend = "none",
...
#Growth curves
ggsave(plot = GCplot, "TPA - growth curves.pdf",
       path = "./", 
       width = 12.5,
       height = 10,
       units='in',
...
#TPCs by treatment
ggsave(plot = TPCplot, "TPA - TPCs all models.pdf", 
       path = "./", 
       width=10,
       height=10,
       units='in',
